...
const xiaomeiUrl = process.env.PUBLIC_URL
  ? `${process.env.PUBLIC_URL}/xiaomei.svg`
  : "/xiaomei.svg";

export const XiaomeiIcon = (
...
async function fetchSuggestions(
  ...
  const res = await fetch(
    `${AI_SERVICE_URL}/v1/assistant/suggestions?role=${role}`,
    {
      headers: {
...
const AIAssistant: React.FC = () => {
  ...
  const startStream = useCallback(async () => {
    ...
    const newItems: ChatItem[] = [
      ...history,
      { role: "user", content: input || `围绕主题：${topic} 给出创意建议` },
    ];
    setItems(newItems);
    setInput("");
    setLoading(true);
                ...
                >
                  <Avatar
                    src={
                      m.role === "assistant"
                        ? xiaomeiUrl
                        : resolveFileUrl((user as any)?.avatar)
                    }
                    icon={<UserOutlined />}
                    size="large"
                  />
