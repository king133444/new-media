...
### 本次更新亮点：AI 助手、广告广场 AI 筛选与作品集流程

- AI 助手（小媒）集成：
  - 在主页面集成了可供 管理员 / 广告商 / 创作者 使用的 AI 助手（前端组件：`fronted/src/components/AIAssistant.tsx`）。助手基于仓内 `ai-service` 微服务提供的统一 LLM 接口，支持流式（SSE）响应与人格（小媒）注入，前端通过环境变量 `REACT_APP_AI_SERVICE_URL` 指向微服务。
  - 助手支持角色感知（creator/advertiser/designer/admin），在打开抽屉时会获取基于角色的快速提问建议，并使用 `/v1/assistant/inspire/stream` 进行流式生成（用户体验为“边输出边显示”）。

- 广告广场的 AI 筛选：
  - 在“广告广场”（Orders / 广告投放广场）新增基于订单需求、创作者技能与标签的 AI 匹配筛选逻辑（后端/前端配合实现，可在申请列表与筛选组件中看到对应调用点）。该筛选可帮助广告商快速定位与订单最匹配的创作者候选集。

- 申请/委派排序策略：
  - 当多个创作者申请同一广告时，广告商在查看“申请/委派”列表时会看到按 综合评分（创作者历史评分）与 技能/标签匹配度 排序的申请列表，确保最适合的创作者排在前面，便于高效委派。

- 作品集管理与审核：
  - 创作者可上传作品至作品集（前端/后端支持上传与关联），管理员可在后台进行审核（通过/拒绝）。审核通过后的作品会在广告商查看申请/委派时显示，且在实时聊天中心的名片或对话中亦可预览。

- 评价管理：
  - 扩展了评价管理逻辑：订单完成/确认后产生待评价记录，支持双方评价、管理员查看全部评价，以及在广告商委派/创作者展示中结合评分作为排序依据。

- AI 微服务封装与获取：
  - AI 微服务位于 `ai-service/`，已提供 `Dockerfile`、`docker compose` 支持与完整的 `ai-service/README.md`（包括环境变量说明、运行示例与多架构镜像构建工作流）。如果需要可直接从仓库的 CI/工作流获取已经构建的多架构镜像（见 `.github/workflows/ai-service-docker.yml`）。
  - 开发者可按 `ai-service/README.md` 中的说明在本地以虚拟环境或 Docker 运行微服务，生产环境推荐使用容器镜像。

以上为本次新增的亮点功能说明，下面保留此前已有的若干更新点。
